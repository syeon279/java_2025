...
public class AopTrace {
   ...
   public Object trace( ProceedingJoinPoint joinPoint ) throws Throwable {
      //1. target의 메서드정보
	   String signature = joinPoint.getSignature().toShortString(); // target의 메서드 정보를 가져옴.
	   System.out.println(">>>>" + signature + " >> start!");
	   
      //2. 호출시간 확인
	   long start = System.currentTimeMillis(); // 시작시간과 끝나는 시간 체크
	   
      //3. target 메서드실행
	   Object result = joinPoint.proceed(); // success or fail?
