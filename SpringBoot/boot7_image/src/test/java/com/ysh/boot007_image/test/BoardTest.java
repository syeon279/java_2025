...
import com.ysh.boot7_image.board.BoardRepository;
import com.ysh.boot7_image.board.BoardService;
...
public class BoardTest {
	...
	public void test() {
		 ...
		 * for(int i=0; i<120; i++) {
		 * 
		 * Member member = new Member(); member.setUsername("sendy"); //Optional<Member>
		 * findmember = memberRepository.findById(2L); //Member member =
		 * findmember.get();
		 * 
		 * Board board = new Board(); board.setBtitle("hello");
		 * board.setBcontent("hihi"); try {
		 * board.setBip(Inet4Address.getLocalHost().getHostAddress()); } catch
		 * (UnknownHostException e) { e.printStackTrace(); } board.setBpass("22");
		 * 
		 * board.setMember(member); boardService.insert(board); }
		 */
	}
