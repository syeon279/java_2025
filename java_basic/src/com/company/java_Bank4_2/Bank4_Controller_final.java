package com.company.java_Bank4_2;
...
// 아이디 삭제 후 조회할 때 없는 계좌 안나옴, 삭제 후 추가 안됨. 
		...
		for(int i=0; i<users.length; i++) {
			if(users[i]!=null){
				if(users[i].getId().equals(tempId) && users[i].getPass().equals(tempPass)) {
					find = i;
					System.out.println("사용자 정보가 확인되었습니다."); break;
				} else {
					System.out.println("계좌 정보가 존재하지 않습니다."); break;
				}
			} else {
				System.out.println("계좌 정보가 존재하지 않습니다."); break;
			} 
...
class Input  implements Bank4_Controller { 
	...
	public void exec(Bank[] users) {
		...
		for(int i=0; i<users.length;i++) { 
			if(users[i] == null) {
				users[i] = new Bank(); //인스턴스 생성
				Scanner scanner = new Scanner(System.in);
				System.out.print("ID 입력: "); 
				users[i].setId(scanner.next());
				System.out.print("비밀번호 입력: ");
				users[i].setPass(scanner.next());
				System.out.print("잔액 입력: "); 
				int tempBalance = scanner.nextInt();
				if(tempBalance<0) {
					result = "유효하지 않은 금액입니다."; 
				} else {
					users[i].setBalance(tempBalance); 
					result = "사용자 정보가 저장되었습니다.";
					break;
				}
			} else {
				result = "더이상 회원 가입을 받지 않습니다.";
			}
		}// E for
	...
	public void exec(Bank[] users) {
		...
		if(find!=-1) {
			...
			if (delacnt == 'y' || delacnt == 'Y') {
				users[find].setId(""); users[find].setPass(""); users[find].setBalance(0);
