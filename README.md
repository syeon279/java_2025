...
```
vi ( 파일명 ) : vi editor 입력모드(텍스트 입력)/ 명령모드(커서 이동, 복사, 저장)
i : 입력모드로 전환
esc : 빠져 나오기
wq! : 파일 저장
```
...
<table>
	...
	<tr>
		<th rowspan="3"> 핵심 구성 요소</th>
		<td>개체(Entity)</td>
		<td>테이블, 개념의 정보 단위, 도메인</td>
	</tr>
	<tr>
		<td>속성(Attribute)</td>
		<td> 테이블 열, 가장 작은 논리적 단위 </td>
	</tr>
	<tr>
 	...
 	<tr>
		<th rowspan="3"> 관계 종류 </th>
		<td> 하나의 A는 하나의 B로 구성되어 있다. </td>
		<td>하나의 학생은 하나의 학과에 속해 있다. </td>
	</tr>
 	<tr>
		<td>하나의 A는 여러개의 B로 구성되어 있다. </td>
		<td>하나의 대학교는 여러개의 학과로 구성되어 있다.</td>
	</tr>
 	<tr>
		<td>여러개의 A는 여러개의 B로 구성되어 있다. </td>
		<td> 과목은 수강하는 많은 학생을 가진다. <br/> 학생은 많은 과목을 수강할 수 있다. </td>
	</tr>
 	<tr>
		<th>실선</th>
		<td>부모 테이블의 기본키를 자식 테이블이 가지고 있고 기본키로 사용</td>
	</tr>
 	<tr>
		<th>점선</th>
		<td>부모 테이블의 기본키를 자식테이블이 가지고 있고 기본키로 사용 안한 경우 </td>
	</tr>
...
1️⃣  1NF : 원자값 - 중복항복 없애기

- 원자값 → 더 이상 쪼개질 수 없는 단위
- 각 속성은 하나의 값만, 순서가 상관이 없어야 함
...
- X를 알면 Y를 알 수 있고, Y를 알면 X를 알 수 있다.
- 다른 후보키에 의존하지 않음

4️⃣  BCNF : 모든 결정자는 후보키( 유일성 + 최소성 )

- 후보키: 유일성 + 최소성

5️⃣  4NF : 다치종속제거

6️⃣  5NF : 조인종속 다대다 관계 → 중간 테이블 필요 

- JOIN 종속: 하나의 테이블을 여러 개의 테이블로 분해하였다가, 다시 조인했을 때 데이터 손실은 없지만 필요없는 데이터가 생기는 것
